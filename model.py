...
class Attention(nn.Module):
    def __init__(self, hidden_size, batch_first=False, device="gpu"):
        ...
        #self.att_weights = self.att_weights.to(self.device)
...
class AttenLSTM(nn.Module):
    def __init__(self, input_size=1024, hidden_size=512, batch_first=True, dropout=0.5, bidirectional=True,
    ...
    #Cannot modify this part, since the weights in the pretrained path is fixed.
    '''
    def forward(self, x):
        out1, (_, _) = self.lstm1(x)
        out2, _ = self.atten1(out1)

        z = self.fc(self.dropout(out2))
        return z
    '''
